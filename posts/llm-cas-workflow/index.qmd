...
$$
and analogously for the charged components $\phi_i^\pm$, $\chi_{L,R}^\pm$. The potential is then expanded to quadratic order in the fluctuations; the bilinear part defines the mass spectrum.
...
1. Formulating the **potential** and the **verification protocol**.
2. Asking an LLM to generate the CAS code.
3. Running the code, checking the `assert`s, and analyzing the limiting cases.
